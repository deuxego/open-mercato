...
| Multi-step durable workflow | Inngest function | `inngest.send()` |
...
The previous spec version routed events through BullMQ → persistent wildcard subscriber → Inngest. This added ~50-100ms latency per hop and required changes to the events worker (wildcard support, `eventName` propagation). The native approach:
...
- **Standard Inngest pattern** — follows their Next.js quickstart exactly
- **Explicit triggering** — developers clearly see when a workflow is triggered vs when an event is emitted

**Coupling concern mitigated by DI:** Modules resolve `inngestClient` from the DI container. If the Inngest package is removed, the DI registration disappears and `resolve('inngestClient')` fails at the call site — no hidden residue. Modules that trigger workflows already know they're orchestrating, not just emitting side effects.
...
│   ├── types.ts                      # WorkflowMeta, WorkflowHandler, WorkflowContext type
...
export class WorkflowContext {
  ...
  async emitToEventBus(name: string, data: Record<string, unknown>): Promise<void> {
    await this.step(`emit:${name}`, async () => {
  ...
  async sendEvent(name: string, data: Record<string, unknown>): Promise<void> {
    await this.inngestStep.sendEvent(`send:${name}`, {
      name,
      data,
    })
  }
...
**Two emission methods — intentionally distinct:**
- `ctx.emitToEventBus()` — fires to Mercato event bus (subscribers, SSE, persistent handlers)
- `ctx.sendEvent()` — fires to Inngest (triggers other workflows). Use sparingly for saga coordination.
...
Workflows are triggered by calling `inngest.send()` — the standard Inngest pattern. The client is available via DI:
...
export default async function handler(
  ...
  // Trigger durable workflow for long-running follow-up
  const inngestClient = ctx.resolve<Inngest>('inngestClient')
  await inngestClient.send({
    name: 'orders.invoice.created',
    data: payload,
...
// Trigger another Inngest workflow (saga coordination)
await ctx.sendEvent('orders.shipping.requested', { orderId, organizationId, tenantId })
```
...
Need to emit to the event bus from within a workflow?
  → ctx.emitToEventBus()
```
...
| `inngest.send()` → workflow | Send event via DI client, verify workflow executes |
...
| `ctx.sendEvent()` | Workflow triggers another workflow |
