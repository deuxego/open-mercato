...
| `integration.ts` | `adapter?` / `adapters?` (optional) | MAY optionally export `adapter` (single instance) or `adapters` (versioned array) for hub auto-registration at bootstrap. Additive — existing files without these exports are unaffected |
