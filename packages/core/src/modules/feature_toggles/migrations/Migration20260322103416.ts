...
export class Migration20260322103416 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table "feature_toggles" ("id" uuid not null default gen_random_uuid(), "identifier" text not null, "name" text not null, "description" text null, "category" text null, "default_value" jsonb not null, "type" text not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "feature_toggles_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table "feature_toggle_overrides" ("id" uuid not null default gen_random_uuid(), "toggle_id" uuid not null, "tenant_id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "value" jsonb not null, constraint "feature_toggle_overrides_pkey" primary key ("id"));`);
