...
export class Migration20260215120000 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    // Drop localized_content column from catalog_product_offers (table may not exist if catalog module was removed)
    this.addSql(`
      do $$
      begin
        if exists (
  ...
  override async down(): Promise<void> {
    // Re-add localized_content column (table may not exist if catalog module was removed)
    this.addSql(`
      do $$
      begin
        if exists (
          ...
          alter table "catalog_product_offers" add column "localized_content" jsonb null;
