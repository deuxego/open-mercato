...
function writeEntityFieldFiles(outRoot: string, fieldsByEntity: EntityFieldMap): Set<string> {
  ...
  for (const [entity, fields] of Object.entries(fieldsByEntity)) {
    ...
    const content = fields.map((f) => `export const ${toVar(f)} = ${JSON.stringify(f)}`).join('\n') + '\n'
...
/**
 * Write the entity-fields-registry.ts file with all field data inlined.
 * No imports from entity directories — avoids Turbopack resolution failures
 * when resolving 70+ sibling directory imports simultaneously.
...
function writeEntityFieldsRegistry(generatedRoot: string, fieldsByEntity: EntityFieldMap): void {
  ...
  const inlinedEntries = entities.map((e) => {
    ...
    const fieldEntries = fields.map((f) => `    ${toVar(f)}: ${JSON.stringify(f)}`).join(',\n')
    return `  ${toVar(e)}: {\n${fieldEntries}\n  }`
  }).join(',\n')
