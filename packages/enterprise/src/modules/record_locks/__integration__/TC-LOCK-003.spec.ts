...
test.describe('TC-LOCK-003: Accept mine conflict resolution and notification', () => {
  ...
  test('should resolve conflict with accept_mine and notify incoming actor', async ({ request }) => {
    ...
    try {
      ...
      todoId = await createTodoFixture(request, adminToken, `QA TC-LOCK-003 Todo ${Date.now()}`);
      ...
      const incomingTitle = `QA TC-LOCK-003 Incoming ${Date.now()}`;
      ...
      const mineTitle = `QA TC-LOCK-003 Keep Mine ${Date.now()}`;
      const resolveAttempt = await updateTodo(
        request,
        superadminToken,
        todoId,
        mineTitle,
