...
## Query extensions (UMES Phase N)

Both engines support an optional `extensions` context that activates sync lifecycle events and query-level enrichers around the core SQL query. When provided, the pipeline runs before-query subscribers, re-applies scope guards, executes enrichers, and runs after-query subscribers.
...
See the full guide: [Query engine extensibility](/framework/extensibility/query-engine-extensibility).
