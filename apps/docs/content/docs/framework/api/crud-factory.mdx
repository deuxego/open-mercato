...
const crud = makeCrudRoute({
  orm: {
    entity: TodoEntity, // MikroORM entity class
...
- Resource path (`/api/example/todos`)
...
crud|example.todo|GET|/api/example/todos|tenant:123|selectedOrg:456|scope:456,789|query:page=1&pageSize=50&search=test
```
...
metadata: {
  ...
  POST: {
    requireAuth: true,
    requireFeatures: ['example.todos.create'],
  },
  // ...
}
```
...
```ts
describe("example.todos.create", () => {
  it("creates person with custom fields", async () => {
    const { result } = await executeCommand("example.todos.create", {
...
```ts
describe("GET /api/example/todos", () => {
  it("returns paginated results", async () => {
    const response = await app.request(
      "/api/example/todos?page=1&pageSize=10"
    );
