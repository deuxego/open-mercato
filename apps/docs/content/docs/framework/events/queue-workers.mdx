...
The `@open-mercato/queue` package provides a multi-strategy job queue system for reliable background processing. It integrates seamlessly with the [Events system](./overview) to handle persistent events asynchronously.
...
| `fulltext-index.worker.ts` | `fulltext-indexing` | Indexes documents for fulltext search |
| `vector-index.worker.ts` | `vector-indexing` | Generates embeddings for vector queries |
