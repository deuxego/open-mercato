...
class MyReportService {
  ...
  async enableAutoReport(config: ReportConfig) {
    await this.schedulerService.register({
      id: `reports:generate:${config.organizationId}`,
      ...
      targetType: 'queue',
      targetQueue: 'report-generation',
      targetPayload: { format: config.format },
      sourceModule: 'reports',
    })
  }

  async disableAutoReport(organizationId: string) {
    await this.schedulerService.unregister(
      `reports:generate:${organizationId}`
    )
  }
}
```
...
- [User Guide: Scheduled Jobs](../../user-guide/scheduler) - End-user documentation
