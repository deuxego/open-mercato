...
- See [Widget injection](../widget-injection) for spot catalogs, event hooks, and examples.
...
- Edit `apps/mercato/src/modules.ts` and list modules to load, e.g.:
  - `{ id: 'auth', from: '@open-mercato/core' }`, `{ id: 'directory', from: '@open-mercato/core' }`, `{ id: 'example', from: '@open-mercato/core' }`.
- Generators and migrations only include these modules.

## Monorepo, Overrides, and Module Config
- Core modules live in `@open-mercato/core`, events in `@open-mercato/events`, etc.
