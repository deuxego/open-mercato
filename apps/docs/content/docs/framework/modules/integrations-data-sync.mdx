...
<Callout type="warn">
`registerDataSyncAdapter()` is deprecated. Use the `integration.ts` export pattern above, or call `dataSyncHub.register()` from `@open-mercato/shared/lib/hub` directly.
</Callout>

Legacy `setup.ts` registration (deprecated):
