...
```typescript
{
  ...
  type: string                 // Notification type (e.g., 'example.todo.pending')
...
```typescript
{
  ...
  actions?: Array<{
    ...
    commandId?: string         // Command to execute (e.g., 'example.todos.approve')
...
actions: [
  {
    ...
    label: 'Approve',
    variant: 'default',
    commandId: 'example.todos.approve', // Command to execute
    icon: 'check',
  },
  {
    id: 'reject',
    label: 'Reject',
    variant: 'destructive',
    commandId: 'example.todos.reject',
...
actions: [
  {
    ...
    label: 'View Details',
    variant: 'outline',
    href: '/backend/example/todos/{sourceEntityId}', // {sourceEntityId} is replaced
    icon: 'external-link',
  },
]
```
...
await notificationService.createForRole({
  ...
  type: 'example.report.ready',
  title: 'Quarterly Sales Report Available',
  body: 'The Q4 2025 activity report is now ready for review',
  icon: 'file-text',
  severity: 'info',
...
await notificationService.createForFeature({
  requiredFeature: 'example.tasks.approve', // The required permission
  type: 'example.task.pending_approval',
  ...
  actions: [
    {
      ...
      label: 'Approve',
      variant: 'default',
      commandId: 'example.tasks.approve',
      icon: 'check',
    },
    {
      id: 'reject',
      label: 'Reject',
      variant: 'destructive',
      commandId: 'example.tasks.reject',
      confirmRequired: true,
      icon: 'x',
    },
  ],
  primaryActionId: 'approve',
  linkHref: '/backend/example/tasks/123',
}, {
  tenantId: ctx.auth.tenantId,
  organizationId: ctx.selectedOrganizationId,
})
...
await apiCall('/api/notifications/feature', {
  method: 'POST',
  body: JSON.stringify({
    requiredFeature: 'example.todos.approve',
    type: 'example.todo.pending_approval',
...
```typescript
// packages/core/src/modules/example/subscribers/leave-request-approval-notification.ts
import { createQueue } from '@open-mercato/queue'
import type { CreateFeatureNotificationJob } from '@open-mercato/core/modules/notifications/workers/create-notification.worker'

export const metadata = {
  event: 'example.task.created',
  id: 'example:task-approval-notification',
  persistent: true,
}
...
export default async function handle(
  ...
  // This includes:
  // - Users with direct "example.tasks.approve" permission
  // - Users with "example.*" wildcard permission
  // - Super admins
  await queue.enqueue({
    type: 'create-feature',
    input: {
      requiredFeature: 'example.tasks.approve',
      type: 'example.task.pending_approval',
      title: 'Leave request pending approval',
      ...
      actions: [
        ...
        },
        {
          ...
          label: 'View Details',
          variant: 'outline',
          href: `/backend/example/tasks/${payload.leaveRequestId}`,
          icon: 'external-link',
        },
      ],
      primaryActionId: 'approve',
      ...
      linkHref: `/backend/example/tasks/${payload.leaveRequestId}`,
...
4. **Wildcard Support**: `example.*` matches any `example.` permission like `example.tasks.approve`

**Example Scenarios:**

- User has role "HR Manager" with permission `example.tasks.approve` → ✅ Gets notification
- User has direct permission `example.*` → ✅ Gets notification (wildcard match)
...
- User has permission `example.todos.approve` → ❌ No notification (different module)
...
await eventBus.emit('example.todo.created', {
  orderId: order.id,
...
```typescript
// packages/core/src/modules/example/subscribers/order-approval-notification.ts
export const metadata = {
  event: 'example.todo.created',
  id: 'example:todo-approval-notification',
  persistent: true,
}
  ...
  for (const manager of managers) {
    await queue.enqueue({
      ...
      input: {
        ...
        actions: [
          {
            ...
            label: 'Approve',
            variant: 'default',
            commandId: 'example.todos.approve',
            icon: 'check',
          },
          {
            id: 'reject',
          ...
          },
          {
            ...
            label: 'View Details',
            variant: 'outline',
            href: `/backend/example/todos/${payload.orderId}`,
            icon: 'external-link',
          },
        ],
        primaryActionId: 'approve',
        ...
        linkHref: `/backend/example/todos/${payload.orderId}`,
...
1. The command `example.todos.approve` is executed
...
```typescript
// packages/core/src/modules/example/notifications.client.ts
'use client'

import type { NotificationTypeDefinition } from '@open-mercato/shared/modules/notifications/types'
import { ExampleTodoCreatedRenderer } from './widgets/notifications/ExampleTodoCreatedRenderer'
import { ExampleTaskCreatedRenderer } from './widgets/notifications/ExampleTaskCreatedRenderer'

export const exampleNotificationTypes: NotificationTypeDefinition[] = [
  {
    type: 'example.todo.created',
    module: 'example',
    titleKey: 'example.notifications.order.created.title',
    icon: 'shopping-cart',
    severity: 'info',
    actions: [
      {
        ...
        variant: 'outline',
        href: '/backend/example/todos/{sourceEntityId}',
      },
    ],
    linkHref: '/backend/example/todos/{sourceEntityId}',
    Renderer: ExampleTodoCreatedRenderer, // Custom renderer component
    expiresAfterHours: 168,
  },
  {
    type: 'example.task.created',
    module: 'example',
    Renderer: ExampleTaskCreatedRenderer,
    // ...other fields
  },
]
...
```tsx
// packages/core/src/modules/example/widgets/notifications/ExampleTodoCreatedRenderer.tsx
'use client'
...
export function ExampleTodoCreatedRenderer({
  notification,
  onAction,
  onDismiss,
...
```tsx
import { exampleNotificationTypes } from '@open-mercato/core/modules/example/notifications.client'
...
The example module demonstrates custom renderers for todo and task creation notifications:

**Order Created Notification:**
- Blue left border to indicate module context
...
Files:
- `packages/core/src/modules/example/notifications.ts` - Server-side type definitions
- `packages/core/src/modules/example/notifications.client.ts` - Client-side with renderers
- `packages/core/src/modules/example/widgets/notifications/ExampleTodoCreatedRenderer.tsx`
- `packages/core/src/modules/example/widgets/notifications/ExampleTaskCreatedRenderer.tsx`
...
type: 'example.todo.pending_approval'
type: 'example.todo.approved'
type: 'example.todo.rejected'
...
- [Events & Subscribers](../events/overview) - Event publishing system
- [Queue & Workers](../events/queue-workers) - Background job processing
- [Commands](../commands/overview) - Command execution from actions
