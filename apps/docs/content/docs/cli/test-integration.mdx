...
- `OM_INTEGRATION_APP_READY_TIMEOUT_SECONDS=<seconds>` increases the ephemeral app readiness timeout (default `90`). This is the knob to raise in slower CI runners, for example `OM_INTEGRATION_APP_READY_TIMEOUT_SECONDS=180 yarn test:integration:coverage`.
...
BASE_URL=http://127.0.0.1:<port> npx playwright test --config .ai/qa/tests/playwright.config.ts admin/TC-ADMIN-001.spec.ts --retries=0
...
yarn test:integration:ephemeral -- --filter admin/TC-ADMIN-001.spec.ts --no-screenshots
```
