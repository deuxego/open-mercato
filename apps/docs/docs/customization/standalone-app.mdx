...
The scaffolded app comes with pre-enabled modules covering common business operations:
...
| File | When to use |
| --- | --- |
| `docker-compose.yml` | **Local development** — starts only infrastructure services (PostgreSQL, Redis). You run the app with `yarn dev` on the host. |
...
- **Docker services not starting** — verify Docker is running and ports 5432 and 6379 are free. Use `docker compose logs` to inspect errors.
