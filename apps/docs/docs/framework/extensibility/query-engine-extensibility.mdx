...
const all = getEnrichersForEntity('example.todo')

// Only enrichers opted in to query-engine pipelines
const queryEngine = getEnrichersForEntity('example.todo', {
  surface: 'query-engine',
  engine: 'basic',
})

// API-response surface (same as no selector)
const apiOnly = getEnrichersForEntity('example.todo', {
...
| Event | When |
|-------|------|
| `example.todo.querying` | Before query execution |
| `example.todo.queried` | After query execution |

Wildcard patterns work: `example.*.querying` matches all example module entity queries.
