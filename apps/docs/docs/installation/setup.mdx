...
2. **Start Docker services**
   ...
   docker compose up -d
   ```
   This starts PostgreSQL 15 with pgvector and Redis 7.
...
6. **Bootstrap data**
   ...
   yarn initialize
   ```
   This script prepares module registries, generates/applies migrations, seeds default roles, provisions an admin user, and loads demo data. Add `--no-examples` if you prefer to skip the demo content while keeping core identities.
7. **Launch the app**
   ```bash
   yarn dev
   ```
...
| `DEMO_MODE` | `true` | Seeds demo data on first run |
...
Run the complete Open Mercato stack with Docker Compose, including the application, PostgreSQL, and Redis. This approach is ideal for quick demos, testing, or containerized deployments.
...
**Note:** The container hostnames (`postgres`, `redis`) match the service names in `docker-compose.fullapp.yml`.
