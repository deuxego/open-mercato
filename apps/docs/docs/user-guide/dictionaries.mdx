...
Need to seed dictionaries in code? Export them from a module command or wire them into your deployment scripts.
...
export const entities = [
  defineFields(
    entityId('auth', 'user'),
    [
      cf.dictionary('compliance_status', 'compliance:statuses', {
...
Replace `'compliance:statuses'` with the `<module>:<dictionary>` identifier you maintain. Inline creation is enabled by default in code; pass `{ dictionaryInlineCreate: false }` to lock the list down.
