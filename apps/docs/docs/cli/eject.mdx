...
`yarn mercato module eject <moduleId>` is an alias for this command, provided for consistency with the `module` command namespace. Both are identical in behavior.
...
| `<moduleId>` | The module identifier to eject (e.g., `example`). |

## Ejectable Modules

Only modules with `ejectable: true` in their metadata can be ejected. Run `yarn mercato eject --list` to see the current set of ejectable modules in your project.
...
Ejectable modules:
  ...
  example (from: @open-mercato/core) — Example module
  ...
...
```bash
yarn mercato eject example
```

Output:

```
Ejecting module "example"...

Module "example" ejected successfully!

Next steps:
  1. Run generators:  yarn mercato generate all
  2. Customize:       edit src/modules/example/
